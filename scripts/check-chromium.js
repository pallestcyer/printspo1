...
async function runChecks() {
  try {
    ...
    if (platform === "linux") {
      try {
        const chromiumVersion = execSync("chromium-browser --version")
          .toString()
          .trim();
        ...
        try {
          ...
          if (chromePath) {
            ...
            if (
              !permissions.endsWith("755") &&
              !permissions.endsWith("775") &&
              !permissions.endsWith("777")
            ) {
            ...
            // Verify it works now
            try {
              const versionCheck = execSync(`${chromePath} --version`)
                .toString()
                .trim();
              console.log(`✅ Browser is now executable: ${versionCheck}`);
            } catch (verifyError) {
              console.log(
                `❌ Browser is still not executable: ${verifyError.message}`,
              );
              hasErrors = true;
            }
          } else {
      ...
      try {
        ...
        if (fs.existsSync(puppeteerCachePath)) {
          ...
          console.log(
            "Puppeteer cache not found, it will be created on first run",
          );
        }
      } catch (cacheErr) {
      ...
      try {
        ...
        if (
          ...
          (available.match(/^\d+(\.\d+)?G$/) && parseFloat(available) < 1)
        ) {
    ...
    try {
      ...
      if (typeof executablePath === "function") {
        ...
        console.log(
          "❌ @sparticuz/chromium executable path function not found",
        );
        hasErrors = true;
      }
    } catch (err) {
    ...
    try {
      ...
      try {
        ...
        if (mainPuppeteerVersion !== puppeteerVersion) {
          console.log(
            `⚠️ Version mismatch between puppeteer (${mainPuppeteerVersion}) and puppeteer-core (${puppeteerVersion})`,
          );
        }
      } catch (mainErr) {
    ...
    if (platform === "linux") {
      try {
        ...
        if (missingLibraries.length > 0) {
          ...
          console.log("\nSuggested fix:");
          console.log(
            `yum install -y ${missingLibraries
              .map((lib) => {
