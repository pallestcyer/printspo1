...
export async function POST(req: Request) {
  try {
    ...
    const { images, printSize, spacing, containMode = false, isPreview = false } = await req.json() as GenerateRequest;
    ...
    try {
      ...
      console.error('Detailed process error:', processError);
      return NextResponse.json(
        { error: 'Failed to process images', details: processError instanceof Error ? processError.message : 'Unknown error' },
        { status: 500 }
      );
    }
  } catch (error) {
